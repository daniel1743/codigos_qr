...
export interface EditorValue {
  ...
  publishing: boolean;
  publish: () => void;
  /** Generic single-commit doc update (one undo step). Used by card-level structure ops. */
  updateDoc: (fn: (d: PageDoc) => PageDoc) => void;
  /** Review flag: exposes the prepared-but-hidden card families in the block picker. */
  showPreparedFamilies: boolean;
}
...
export function EditorProvider({ children, initialTemplate = 'bio', initialDevice = 'desktop', showPreparedFamilies = false }: EditorProviderProps) {
  ...
  const select = useCallback(
    (id: string, opts?: {reveal?: boolean;}) => {
      ...
      setSheet('compact');
      setKeyboard(false);
      setEditingId(!isMobile && (r.kind === 'text' || r.kind === 'price') ? id : null);
