...
export function familyCardActions(ed: EditorValue, ctx: CardContext): EditorAction[] {
  ...
  return [
  {
    ...
    showLabel: true,
    panel: <CardLayoutPicker options={layoutsForFamily(family)} value={r.layout} onChange={(v: CardLayout) => set('layout', v)} />
  },
  {
    key: 'imagePos',
    label: 'Imagen',
    ...
    <div className="space-y-4">
          ...
          <PanelSection title="Proporción" hint={horizontal ? 'Imagen / contenido.' : 'Disponible con imagen a la izquierda o derecha.'}>
            <Segmented
          ...
          onChange={(v) => {
            ed.updateDoc((d) => ({
              ...d,
              props: { ...d.props, [cardId]: { ...d.props[cardId], ratio: v, layout: horizontal ? r.layout === 'balanced' ? 'left' : r.layout : 'left' } }
            }));
          }} />
...
export function priceActions(ed: EditorValue, t: ThemeTokens, id: string, ctx: CardContext | null, el: HTMLElement | null): EditorAction[] {
  ...
  { key: 'color', label: 'Color', icon: PaletteIcon, swatch: ts.color, panel: <SwatchRow colors={t.swatches} value={ts.color} onChange={(c) => ed.setTextStyle(id, { color: c })} /> },
  {
    key: 'hide',
    label: 'Ocultar',
    icon: EyeOffIcon,
...
export function badgeActions(ed: EditorValue, id: string, ctx: CardContext | null, el: HTMLElement | null): EditorAction[] {
  ...
  { key: 'remove', label: 'Quitar', icon: Trash2Icon, danger: true, onClick: () => ed.removeElement(id, 'Etiqueta') }];
