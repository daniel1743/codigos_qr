...
import { blockKit, blockLabels, preparedBlockKit } from '../../data/blockKit';
...
export function BlockPicker({ variant }: BlockPickerProps) {
  ...
  const grid = (items: typeof blockKit) =>
  ...
  <>
      {grid(blockKit)}
      {ed.showPreparedFamilies && preparedBlockKit.length > 0 &&
    <div className="mt-5 border-t border-dashed border-line pt-4">
          <p className="mb-2.5 text-[12px] font-medium text-mute">Familias preparadas · ocultas en producción</p>
          {grid(preparedBlockKit)}
        </div>
    }
    </>;
