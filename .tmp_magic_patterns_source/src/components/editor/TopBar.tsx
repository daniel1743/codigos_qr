...
export function TopBar() {
  ...
  const isSystem = location.pathname.startsWith('/sistema');
    ...
    <header className="relative z-50 flex h-14 shrink-0 items-center gap-2 border-b border-line bg-white px-3 lg:gap-3 lg:px-4">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <nav className="flex rounded-[10px] bg-[#F2F3F5] p-0.5" aria-label="Vista">
          ...
          <NavLink to="/sistema" className={({ isActive }) => tab(isActive)}>
            Sistema
          </NavLink>
        </nav>
      </div>
      ...
      {isSystem &&
      <NavLink to="/" className="ml-auto inline-flex h-9 items-center rounded-[10px] bg-ink px-4 text-[13px] font-semibold text-white transition-opacity duration-150 hover:opacity-90">
          Abrir editor
        </NavLink>
      }
    </header>);
