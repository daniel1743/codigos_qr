...
: ResolvedCard {
  const variant: CardVariantDef = family.variants.find((v) => v.id === blockProps.variant) ?? family.variants[0];
  const layout = cardProps.layout as CardLayout ?? variant.layout;
  const ratio = cardProps.ratio as CardRatio ?? variant.ratio ?? (layout === 'balanced' ? '50' : '25');
...
export function layoutsForFamily(family: CardFamilyDef): {value: CardLayout;label: string;}[] {
  ...
  if (family.variants.some((v) => v.layout === 'beforeAfter')) extra.push({ value: 'beforeAfter', label: 'Antes / después' });
  if (family.variants.some((v) => v.layout === 'highlight')) extra.push({ value: 'highlight', label: 'Destacada' });
