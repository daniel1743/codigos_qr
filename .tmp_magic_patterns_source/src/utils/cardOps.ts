...
export function moveCard(d: PageDoc, blockKey: string, count: number, itemId: string, dir: -1 | 1): PageDoc {
...
export function deleteCard(d: PageDoc, blockKey: string, count: number, itemId: string): PageDoc {
  return withOrder(d, blockKey, cardOrder(d, blockKey, count).filter((x) => x !== itemId));
}
...
export function duplicateCard(d: PageDoc, blockKey: string, count: number, prefix: string, itemId: string): PageDoc {
  const order = cardOrder(d, blockKey, count);
  const i = order.indexOf(itemId);
  if (i < 0) return d;
