...
/**
 * Retail surfaces. Presentation only: every label below is either structural
 * or comes verbatim from the normalized content.
 */
...
export function CategoryShowcase({
  ...
  return (
    <div className="sp-cattiles" role="group" aria-label="Categories">
      {tiles.map((tile) => (
        <button
          key={tile.categoryId}
          type="button"
          className={`sp-cattile${activeId === tile.categoryId ? " sp-cattile--active" : ""}`}
          aria-pressed={activeId === tile.categoryId}
          onClick={() => onSelect(activeId === tile.categoryId ? null : tile.categoryId)}
        >
          <Media media={tile.media} className="sp-cattile__media" ratio="1 / 1" />
          <span className="sp-cattile__label">{tile.label}</span>
          <span className="sp-cattile__count">
            {tile.itemCount} {tile.itemCount === 1 ? "item" : "items"}
          </span>
        </button>
      ))}
    </div>
...
export function CollectionBanner({
  ...
  return (
    <div className="sp-banner">
      <div className="sp-banner__body">
        <p className="sp-banner__eyebrow">Collection</p>
        <h3 className="sp-banner__title">{title}</h3>
        {description ? <p className="sp-banner__desc">{description}</p> : null}
        <div className="sp-banner__actions">
          {actionLabel && onAction ? (
            <button type="button" className="sp-btn sp-btn--primary" onClick={onAction}>
              {actionLabel}
            </button>
          ) : null}
          {cta}
      ...
      <div className="sp-banner__figure">
        <Media media={media} className="sp-banner__media" ratio="16 / 10" />
      </div>
    </div>
  );
}

/** Horizontal, touch-friendly product rail used for secondary collections. */
export function ProductRail({
  ...
  return (
    <ul className="sp-rail">
      {items.map((item) => (
        <li className="sp-rail__item" key={item.id}>
          <ItemCard
            item={item}
            variant={variant}
            action={actionFor(item)}
...
export function BenefitStrip({
  ...
  benefits: Array<{ id: string; title: string; description?: string | undefined }>;
}) {
  if (benefits.length === 0) return null;
  return (
    <ul className="sp-benefits">
      {benefits.map((benefit) => (
        <li className="sp-benefit" key={benefit.id}>
          <span className="sp-benefit__dot" aria-hidden="true" />
          <div>
            <p className="sp-benefit__title">{benefit.title}</p>
            {benefit.description ? <p className="sp-benefit__desc">{benefit.description}</p> : null}
          </div>
        </li>
      ))}
    </ul>
