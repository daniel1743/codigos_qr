...
export async function qaRequest(
  ...
  return page.evaluate(
    ...
    {
      url: qaEnv("VITE_SUPABASE_URL").replace(/\/$/, ""),
      anonKey: qaEnv("VITE_SUPABASE_ANON_KEY"),
      path,
      method,
      body,
    },
...
export async function login(page: Page, baseURL: string): Promise<void> {
  ...
  await expect(
    email.or(page.getByRole("button", { name: "Guardar borrador", exact: true }).first()),
  ...
  await expect(
    page.getByRole("button", { name: "Guardar borrador", exact: true }).first(),
  ).toBeVisible({ timeout: 45_000 });
...
export async function snapshot(page: Page): Promise<QaSnapshot> {
  const profiles = (await qaRequest(
    page,
    `rest/v1/profiles?select=*&user_id=eq.${QA_USER_ID}`,
  )) as QaRecord[];
  ...
  const links = (await qaRequest(
    page,
    `rest/v1/profile_links?select=*&profile_id=eq.${QA_PROFILE_ID}&order=sort_order.asc,id.asc`,
  )) as QaRecord[];
...
export async function restoreBaseline(page: Page, baseline: QaSnapshot): Promise<void> {
  ...
  await qaRequest(
    page,
    `rest/v1/profiles?id=eq.${QA_PROFILE_ID}&user_id=eq.${QA_USER_ID}`,
    "PATCH",
    {
  ...
  for (const link of current.links) {
    if (!baselineIds.has(link.id))
      await qaRequest(
        page,
        `rest/v1/profile_links?id=eq.${link.id}&profile_id=eq.${QA_PROFILE_ID}`,
        "DELETE",
      );
  }
