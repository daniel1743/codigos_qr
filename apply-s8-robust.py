...
registry_code = """
        ...
        window.addSocial = function(platformId) {
        ...
        window.toggleSocial = function(id) {
        ...
        window.updateSocialUrl = function(id, url) {
            const item = appState.socials.items.find(s => s.id === id);
            if (item) {
                item.url = url;
                updateContent();
        ...
        window.renderSocialsEditor = function() {
            ...
            appState.socials.items.forEach((item, index) => {
                ...
                el.innerHTML = `
                    ...
                    <div class="flex-1 flex flex-col justify-center">
                        ...
                        <input type="text" placeholder="${platform ? platform.placeholder : ''}" value="${item.url}" oninput="window.updateSocialUrl('${item.id}', this.value)" class="w-full bg-transparent text-xs text-white focus:outline-none">
                    </div>
                    <button onclick="window.toggleSocial('${item.id}')" class="w-7 h-7 flex items-center justify-center rounded bg-slate-900 text-slate-400 hover:text-white" title="${item.enabled ? 'Desactivar' : 'Activar'}">
                        <i class="fa-solid ${item.enabled ? 'fa-eye' : 'fa-eye-slash'}"></i>
                    </button>
                    <button onclick="window.removeSocial('${item.id}')" class="w-7 h-7 flex items-center justify-center rounded bg-red-900/30 text-red-400 hover:bg-red-900/50" aria-label="Eliminar">
...
# Insert registry before `document.addEventListener('DOMContentLoaded', () => {`
if 'SOCIAL_PLATFORMS =' not in content:
    content = content.replace("document.addEventListener('DOMContentLoaded', () => {", registry_code + "\n          document.addEventListener('DOMContentLoaded', () => {")

# 1. Update normalizeTemplateConfig
# Find window.normalizeTemplateConfig = function(config) { and add S8 normalization
normalize_addition = """              // S8: Normalize Socials
...
"""
if '// S8: Normalize Socials' not in content:
    content = content.replace('const validation = window.validateTemplateConfig(config);', normalize_addition + '\n              const validation = window.validateTemplateConfig(config);')

# 2. Add SortableJS for Socials
sortable_addition = """
            const socialsListEl = document.getElementById('socials-list');
            if (socialsListEl) {
                new Sortable(socialsListEl, {
                    animation: 150,
                    handle: '.drag-handle',
                    onEnd: function (evt) {
...
"""
if 'socialsListEl' not in content:
    content = content.replace("new Sortable(document.getElementById('buttons-list')", sortable_addition + "\n            new Sortable(document.getElementById('buttons-list')")

# 3. Add to syncControlsFromState
if 'renderSocialsEditor();' not in content:
    content = content.replace("document.getElementById('input-footer-text').value = appState.content.footerText;", "document.getElementById('input-footer-text').value = appState.content.footerText;\n            renderSocialsEditor();")

# 4. New render logic for socials
...
if 'let hasSocials = false;' in content:
                          ...
                          <div class="flex justify-between items-center">
                              ...
                              <div class="relative group">
                                  ...
                                  <div class="absolute right-0 mt-1 w-48 bg-slate-900 border border-slate-700 rounded-lg shadow-xl hidden group-hover:block z-50 p-2 grid grid-cols-2 gap-1">
                                      <button onclick="window.addSocial('instagram')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-brands fa-instagram"></i> Instagram</button>
                                      <button onclick="window.addSocial('facebook')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-brands fa-facebook-f"></i> Facebook</button>
                                      <button onclick="window.addSocial('tiktok')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-brands fa-tiktok"></i> TikTok</button>
                                      <button onclick="window.addSocial('youtube')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-brands fa-youtube"></i> YouTube</button>
                                      <button onclick="window.addSocial('linkedin')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-brands fa-linkedin-in"></i> LinkedIn</button>
                                      <button onclick="window.addSocial('twitter')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-brands fa-x-twitter"></i> X</button>
                                      <button onclick="window.addSocial('whatsapp')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-brands fa-whatsapp"></i> WhatsApp</button>
                                      <button onclick="window.addSocial('website')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2"><i class="fa-solid fa-globe"></i> Website</button>
                                      <button onclick="window.addSocial('email')" class="text-[10px] text-left px-2 py-1.5 hover:bg-slate-800 rounded text-slate-300 flex items-center gap-2 col-span-2"><i class="fa-regular fa-envelope"></i> Email</button>
                                  </div>
                              </div>
                          </div>
                          <p class="text-[10px] text-slate-400">Añade y ordena los iconos sociales.</p>
...
print("Applied strict S8 modifications.")
