-- =============================================================================
-- Cripqer Billing Persistence Restore (P0)
--
-- Restores the canonical, provider-neutral billing persistence module after the
-- production baseline. Deterministic and idempotent: safe to run on current
-- production (objects were absent) and safe to re-run.
--
-- Objects:
--   billing_customers, billing_subscriptions, billing_checkouts, billing_events
--   claim_billing_event (atomic provider-scoped idempotency RPC)
--
-- Access model: server-only. Browser roles (anon/authenticated) have NO direct
-- billing access; service_role bypasses RLS and is the only granted role.
-- =============================================================================
