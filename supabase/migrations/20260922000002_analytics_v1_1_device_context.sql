...
-- the browser helper boundary. It is strictly additive:
--   * The RPC signature gains a single `p_device_type TEXT DEFAULT NULL` arg.
--   * The server-side value is validated against the existing
--     `qr_analytics.device_type` CHECK contract (mobile/desktop/tablet/unknown);
--     any other value is stored as NULL (never fabricated).
--   * No fingerprinting, no precise device models, no new PII columns.
-- =============================================================================

-- Remove the previous 11-argument signature so the canonical write boundary has
-- exactly ONE overload (the 12-argument one below). `CREATE OR REPLACE` only
-- replaces a function of the *same* signature, so the old signature must be
-- dropped explicitly.
DROP FUNCTION IF EXISTS public.track_analytics_event(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT);
...
BEGIN
  ...
  -- 6) Insert the bounded, validated event.
...
REVOKE ALL ON FUNCTION public.track_analytics_event(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.track_analytics_event(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT) TO anon, authenticated;
