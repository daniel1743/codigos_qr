...
pattern_loaduser = r"const \[loadedProfile, loadedEntitlements\] = await Promise\.all\(\[\s*profileService\.getProfileByUserId\(supabase, currentSession\.user\.id\)\.catch\(\(\) => null\),\s*getUserEntitlements\(currentSession\.user\.id\)\.catch\(\(\) => null\),\s*\]\);\s*if \(cancelled\) return;\s*setProfile\(loadedProfile\);\s*setEntitlements\(getPremiumOverrideByEmail\(email\) \|\| loadedEntitlements\);"
