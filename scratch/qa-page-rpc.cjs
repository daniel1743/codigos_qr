...
(async () => {
  ...
  let r = await fetch(base + '/rest/v1/pages?select=public_id,title,published,published_template_config&or=(published.eq.false,published_template_config.is.null)&limit=5', { headers: srH });
  console.log('HTTP', r.status, (await r.text()).slice(0,800));

  console.log('=== ANON: direct table SELECT still blocked? ===');
  r = await fetch(base + '/rest/v1/pages?select=public_id&limit=5', { headers: { apikey: anon, Authorization: 'Bearer ' + anon } });
