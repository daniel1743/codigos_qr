...
(async () => {
  ...
  let authed = false;
  try { await ap.waitForFunction(() => document.cookie.includes('sb-'), { timeout: 30000 }); authed = true; } catch {}
  out.auth = authed;
  await ap.close();
  if (!authed) { out.fatal = 'auth failed'; await browser.close(); console.log(JSON.stringify(out, null, 2)); return; }
  ...
  try {
    ...
    editorMounted = false;
    editorAvailable = await page.evaluate(() => document.body.innerText.includes('Power Editor no disponible'));
  }
  ...
  out.page_edit = {
    ...
    page_target_confirmed: !!editorText && editorText.startsWith('Página:') && editorText.includes(PUBLIC_ID),
