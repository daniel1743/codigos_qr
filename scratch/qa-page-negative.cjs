...
(async () => {
  ...
  let r = await fetch(base + "/rest/v1/pages", {
    method: "POST",
  ...
  r = await fetch(base + "/rest/v1/pages?public_id=eq." + PUB_UNPUB, {
    method: "PATCH",
    headers: anonH,
  ...
  // 4. Cleanup fixtures
  console.log("=== cleanup ===");
  r = await fetch(base + "/rest/v1/pages?public_id=in.(" + PUB_UNPUB + "," + PUB_NULL + ")", {
    method: "DELETE",
    headers: srH,
  });
