...
(async () => {
  ...
  let r = await fetch(base + "/rest/v1/pages?select=*&public_id=eq.yfLEdka", { headers: srH });
  ...
  r = await fetch(
    base +
      "/rest/v1/profiles?select=id,user_id,public_id,slug,published,published_revision,published_template_config&public_id=eq.sY9wHGm",
    { headers: srH },
  );
      ...
      JSON.stringify(
        {
          ...
          published_template_config_keys: p.published_template_config
            ? Object.keys(p.published_template_config)
            : null,
        },
        null,
        2,
