...
(async () => {
  ...
  if (!auth.access_token) {
    console.log("SIGNIN FAIL", r.status, JSON.stringify(auth).slice(0, 200));
    return;
  }
  ...
  r = await fetch(base + "/rest/v1/pages?select=public_id,title,published&limit=50", {
    headers: uH,
  });
  const list = await r.json();
  console.log(
    "owner list pages HTTP",
    r.status,
    "| count:",
    Array.isArray(list) ? list.length : "ERR",
  );
  console.log(
    "  contains yfLEdka:",
    Array.isArray(list) && list.some((p) => p.public_id === "yfLEdka"),
  );

  // 2. owner read own page yfLEdka
  r = await fetch(base + "/rest/v1/pages?select=public_id,title,published&public_id=eq.yfLEdka", {
    headers: uH,
  });
  ...
  r = await fetch(base + "/rest/v1/pages", {
    method: "POST",
  ...
  r = await fetch(base + "/rest/v1/pages?public_id=eq." + PUB, {
    method: "PATCH",
    headers: uH,
    body: JSON.stringify({ published: true, published_template_config: { qa: true } }),
  });
  console.log("owner publish draft:", r.status);
  r = await fetch(base + "/rest/v1/pages?select=public_id,published&public_id=eq." + PUB, {
    headers: uH,
  });
  ...
  r = await fetch(
    base +
      "/rest/v1/pages?select=public_id,title,published,published_revision&public_id=eq.yfLEdka",
    { headers: srH },
  );
  console.log("yfLEdka final state:", await r.text());

  // verify no lingering qarpcownr rows
  r = await fetch(base + "/rest/v1/pages?select=public_id&public_id=like.qarpcownr*", {
    headers: srH,
  });
