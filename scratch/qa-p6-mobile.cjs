...
(async () => {
  const out = {};
  // set alias via REST
  await fetch(base + "/rest/v1/pages?public_id=eq." + PUBLIC_ID, {
    method: "PATCH",
    headers: srH,
  ...
  out.owner = {
    alias_section: await page.evaluate(() =>
      document.body.innerText.includes("Enlace personalizado"),
    ),
    input_fits: await page
      .locator("#page_alias")
      .isVisible()
      .catch(() => false),
  ...
  const r = await pp.goto(BASE + "/pg/a/" + ALIAS, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  await pp.waitForTimeout(2000);
  const o2 = await pp.evaluate(() => ({
    sw: document.documentElement.scrollWidth,
    cw: document.documentElement.clientWidth,
  }));
  out.public_alias = {
