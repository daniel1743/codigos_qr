...
(async () => {
  ...
  out.public = {};
  {
    ...
    out.public.stable_page = r && r.status();
    await p.close();
  }
  {
    const p = await browser.newPage();
  ...
  out.alias_ui = {
    section_present: await page.evaluate(() =>
      document.body.innerText.includes("Enlace personalizado"),
    ),
  ...
  const ar = await pubPage.goto(BASE + "/pg/a/" + ALIAS1, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  ...
  out.alias_public1 = {
    ...
    renderer_mounted: await pubPage.evaluate(() =>
      Boolean(document.querySelector('[class*="pts-"], main')),
    ),
    editor_chrome: await pubPage.evaluate(() =>
      Boolean(document.querySelector('[data-testid="power-editor"], .basic-editor-shell')),
    ),
  ...
  const r1 = await pubPage2.goto(BASE + "/pg/a/" + ALIAS1, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  out.old_alias_after_rename = r1 && r1.status();
  const r2 = await pubPage2.goto(BASE + "/pg/a/" + ALIAS2, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  ...
  const r3 = await pubPage3.goto(BASE + "/pg/a/" + ALIAS2, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  ...
  const r4 = await p2.goto(BASE + "/pg/" + PUBLIC_ID, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  ...
  out.data = {
    ...
    profile: {
      public_id: prf.public_id,
      slug: prf.slug,
      published_revision: prf.published_revision,
    },
  };
