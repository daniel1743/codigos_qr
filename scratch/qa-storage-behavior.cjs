...
(async () => {
  ...
  r = await fetch(base + '/storage/v1/object/avatars/qa-anon-test.txt', { method: 'POST', headers: { ...anonH, 'Content-Type': 'text/plain' }, body });
  ...
  if (!auth.access_token) { console.log('SIGN-IN FAILED HTTP', r.status, JSON.stringify(auth).slice(0,200)); return; }
  ...
  r = await fetch(base + '/storage/v1/object/avatars/' + qaPath, { method: 'POST', headers: { ...userH, 'Content-Type': 'text/plain' }, body });
  ...
  r = await fetch(base + '/storage/v1/object/avatars/' + foreignPath, { method: 'POST', headers: { ...userH, 'Content-Type': 'text/plain' }, body });
  console.log('owner upload foreign   : HTTP', r.status, (await r.text()).slice(0,120));

  // banners owner test
  r = await fetch(base + '/storage/v1/object/banners/' + qaPath, { method: 'POST', headers: { ...userH, 'Content-Type': 'text/plain' }, body });
  console.log('owner upload banner    : HTTP', r.status, (await r.text()).slice(0,120));

  // encrypted-documents owner test
  r = await fetch(base + '/storage/v1/object/encrypted-documents/' + qaPath, { method: 'POST', headers: { ...userH, 'Content-Type': 'text/plain' }, body });
  console.log('owner upload enc-doc   : HTTP', r.status, (await r.text()).slice(0,120));

  // cleanup own QA file
  console.log('=== CLEANUP ===');
  r = await fetch(base + '/storage/v1/object/avatars/' + qaPath, { method: 'DELETE', headers: userH });
  console.log('owner delete own file  : HTTP', r.status, (await r.text()).slice(0,80));
  r = await fetch(base + '/storage/v1/object/banners/' + qaPath, { method: 'DELETE', headers: userH });
  console.log('owner delete banner    : HTTP', r.status, (await r.text()).slice(0,80));
  r = await fetch(base + '/storage/v1/object/encrypted-documents/' + qaPath, { method: 'DELETE', headers: userH });
