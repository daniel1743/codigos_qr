...
(async () => {
  ...
  out.public = {};
  {
    ...
    const resp = await page.goto(BASE + "/pg/" + PUBLIC_ID, {
  ...
  {
    const page = await browser.newPage();
    const resp = await page.goto(BASE + "/p/" + PROFILE_PUBLIC_ID, {
      waitUntil: "domcontentloaded",
      timeout: 90000,
    });
  ...
  const authErrors = [];
  authPage.on("console", (m) => {
    if (m.type() === "error") authErrors.push("console: " + m.text());
  });
  ...
  try {
    await authPage.waitForSelector("#email", { timeout: 30000 });
  } catch {}
  if (
    await authPage
      .locator("#email")
  ...
  const listErrors = [];
  listPage.on("console", (m) => {
    if (m.type() === "error") listErrors.push("console: " + m.text());
  });
  listPage.on("pageerror", (e) => listErrors.push("pageerror: " + e.message));

  // test_1: /pages
  const r1 = await listPage.goto(BASE + "/pages", {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  ...
  out.pages_new = {
    ...
    mis_paginas_list_replacing: await listPage.evaluate(
      () => document.querySelector("h1")?.textContent?.trim() === "Mis páginas",
    ),
  ...
  // test_3: /pages/$pageId
  await listPage.goto(BASE + "/pages/" + PAGE_ID, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  await listPage.waitForTimeout(3000);
  out.page_detail = {
    ...
    mis_paginas_list_shown: await listPage.evaluate(() =>
      document.body.innerText.includes("Mis páginas"),
    ),
    navbars: await listPage.locator("header[data-platform-navbar]").count(),
    not_found: await listPage.evaluate(() =>
  ...
  await listPage.goto(BASE + "/pages/" + PAGE_ID + "/edit", {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  ...
  out.page_edit = {
    url: listPage.url(),
    editor_mounted: await listPage.evaluate(() =>
      Boolean(
        document.querySelector(
          '[data-testid="power-editor"], [data-testid="editor-shell"], .power-editor, [class*="editor"]',
        ),
      ),
    ),
    mis_paginas_list_shown: await listPage.evaluate(() =>
  ...
  const qrErrors = [];
  qrPage.on("console", (m) => {
    if (m.type() === "error") qrErrors.push("console: " + m.text());
  });
  ...
  out.qr.panel = {
    ...
    canvas_present: await qrPage.evaluate(() =>
      Boolean(document.getElementById("page-qr-code-canvas")),
    ),
    svg_element_present: await qrPage.evaluate(() =>
  ...
  await qrPage.waitForTimeout(2000);
  out.qr.save = {
    success_toast: await qrPage.evaluate(() =>
      document.body.innerText.includes("Diseño del QR guardado"),
    ),
    runtime_errors: qrErrors,
  };
  ...
  await qrPage.getByRole("button", { name: "QR / Compartir" }).first().click();
  await qrPage.waitForTimeout(2000);
  const fgAfterRefresh = await qrPage
  ...
  out.qr.refresh = {
    ...
    destination_still_pg_yfLEdka:
      !!destAfterRefresh && destAfterRefresh.endsWith("/pg/" + PUBLIC_ID),
    runtime_errors: qrErrors,
  };
  ...
  try {
    ...
    out.export.png = {
      suggested: pngDl.suggestedFilename(),
      size: pngSize,
      non_empty: pngSize > 0,
    };
  } catch (e) {
  ...
  try {
    ...
    out.export.svg = {
      suggested: svgDl.suggestedFilename(),
      size: svgSize,
      non_empty: svgSize > 0,
    };
  } catch (e) {
  ...
  await mobilePage.goto(BASE + "/pages/" + PAGE_ID, {
    waitUntil: "domcontentloaded",
    timeout: 90000,
  });
  ...
  out.mobile = {
    ...
    qr_panel_open: await mobilePage.evaluate(() =>
      document.body.innerText.includes("Destino del QR"),
    ),
    canvas_present: await mobilePage.evaluate(() =>
      Boolean(document.getElementById("page-qr-code-canvas")),
    ),
