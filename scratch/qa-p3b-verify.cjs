...
function checkConfig(label, envelope) {
  ...
  result.valid =
    result.hasThemeColors &&
    result.hasThemeTypography &&
    result.hasLayoutResponsive &&
    result.blocksIsArray;
...
(async () => {
  const r = await fetch(
    base +
      "/rest/v1/pages?select=public_id,published,published_revision,template_config,published_template_config&public_id=eq.yfLEdka",
    { headers: srH },
  );
  ...
  const r2 = await fetch(
    base +
      "/rest/v1/profiles?select=id,public_id,published,published_revision&public_id=eq.sY9wHGm",
    { headers: srH },
  );
  ...
  console.log(
    ...
    JSON.stringify({
      ...
      published_revision: pr.published_revision,
    }),
  );
})();
