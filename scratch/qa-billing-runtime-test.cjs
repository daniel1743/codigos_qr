...
if (!base || !key) { console.error('MISSING_ENV'); process.exit(1); }
...
async function delEvent(eventId) {
  const res = await fetch(`${base}/rest/v1/billing_events?provider=eq.${provider}&event_id=eq.${encodeURIComponent(eventId)}`, { method: 'DELETE', headers });
  return res.status;
}

(async () => {
  console.log('=== TABLE PRESENCE (service_role HEAD) ===');
  for (const t of ['billing_customers', 'billing_subscriptions', 'billing_checkouts', 'billing_events']) {
