...
(async () => {
  ...
  // ---- PHASE 8 + 6: public child page, mobile viewport ----
  {
    ...
    const resp = await page.goto(BASE + '/pg/yfLEdka', { waitUntil: 'domcontentloaded', timeout: 60000 });
    await page.waitForTimeout(2000);
    const overflow = await page.evaluate(() => ({ sw: document.documentElement.scrollWidth, cw: document.documentElement.clientWidth }));
    out.mobile = {
      route: '/pg/yfLEdka (390x844)',
      http: resp && resp.status(),
      renderer_mounted: await page.evaluate(() => document.documentElement.innerHTML.includes('pts-') || document.querySelector('[style*="--pts"]') !== null),
  ...
  // ---- PHASE 8: 404 page ----
  {
    ...
    const resp = await page.goto(BASE + '/pg/qa-does-not-exist', { waitUntil: 'domcontentloaded', timeout: 60000 });
  ...
  // ---- PHASE 2: PageQrPanel runtime (authenticated) ----
  {
    ...
    try { await page.waitForSelector('#email', { timeout: 20000 }); } catch {}
    ...
    try {
      await page.waitForFunction(() => /Canonical cargado|AÑADIR CONTENIDO|Perfil:|Vista previa/.test(document.body.innerText), { timeout: 25000 });
      authConfirmed = true;
    } catch { authConfirmed = false; }
    out.auth_confirmed = authConfirmed;
    out.auth_cookie_sb = await page.evaluate(() => document.cookie.includes('sb-'));

    const resp = await page.goto(BASE + '/pages', { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    const allH1 = await page.locator('h1').allTextContents().catch(() => []);
    out.page_detail = {
      url_after: page.url(),
      h1_list: allH1,
      title: allH1[0] ?? null,
      has_qr_share_button: await page.getByRole('button', { name: 'QR / Compartir' }).count() > 0,
      not_found_msg: await page.evaluate(() => /No se encontró esta página/i.test(document.body.innerText)),
    };
    ...
    if (out.page_detail.has_qr_share_button) {
      ...
      out.pageqr = {
        ...
        canvas_present: await page.evaluate(() => Boolean(document.getElementById('page-qr-code-canvas'))),
        svg_element_present: await page.evaluate(() => Boolean(document.getElementById('page-qr-code-svg'))),
        runtime_errors: errors,
      };
    }
