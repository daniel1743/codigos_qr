...
(async () => {
  ...
  let r = await fetch(
    base + "/rest/v1/pages?select=id,public_id,qr_config&public_id=eq." + PUBLIC_ID,
    { headers: srH },
  );
  const pageRow = await r.json();
  out.column_exists =
    r.status === 200 && Array.isArray(pageRow) && pageRow.length > 0 && "qr_config" in pageRow[0];
  ...
  r = await fetch(
    base + "/rest/v1/pages?select=public_id&qr_config=cs.{}&public_id=eq." + PUBLIC_ID,
    { headers: srH },
  );
  ...
  r = await fetch(
    base +
      "/rest/v1/pages?select=public_id,qr_config->>qr_foreground_color&public_id=eq." +
      PUBLIC_ID,
    { headers: srH },
  );
  ...
  r = await fetch(base + "/rest/v1/pages?select=public_id&public_id=eq." + PUBLIC_ID, {
    headers: anonH,
  });
  ...
  r = await fetch(base + "/rest/v1/pages?public_id=eq." + PUBLIC_ID, {
    method: "PATCH",
    headers: anonH,
    body: JSON.stringify({ qr_config: { qr_foreground_color: "#000000" } }),
  });
  ...
  async function rpc(pubid) {
    const rr = await fetch(base + "/rest/v1/rpc/get_public_page_by_public_id", {
      method: "POST",
      headers: anonH,
    ...
    return {
      ...
      keys: Array.isArray(rows) && rows[0] ? Object.keys(rows[0]).sort() : null,
    };
  }
  ...
  r = await fetch(
    base +
      "/rest/v1/pages?select=id,owner_user_id,profile_id,public_id,title,page_type,template_config,published_template_config,published,published_revision,published_at,slug,qr_config,created_at,updated_at&public_id=eq." +
      PUBLIC_ID,
    { headers: srH },
  );
  ...
  out.page_before = full
    ? {
        ...
        published_template_config_keys: full.published_template_config
          ? Object.keys(full.published_template_config)
          : null,
      }
    : null;
  ...
  r = await fetch(
    base +
      "/rest/v1/profiles?select=id,user_id,public_id,published,published_revision,qr_foreground_color,qr_background_color,qr_dots_type,qr_logo_enabled,qr_gradient&public_id=eq." +
      PROFILE_PUBLIC_ID,
    { headers: srH },
  );
