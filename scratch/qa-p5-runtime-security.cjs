...
const CANONICAL_FIELDS = ['template_config', 'published_template_config', 'published', 'published_revision', 'public_id', 'title', 'page_type'];
const PROFILE_QR_FIELDS = ['qr_foreground_color', 'qr_background_color', 'qr_dots_type', 'qr_logo_enabled', 'qr_gradient', 'qr_logo_url'];
...
(async () => {
  ...
  out.owner_signin = { status: r.status, uid: auth.user && auth.user.id, has_token: Boolean(auth.access_token) };
  if (!auth.access_token) { console.log(JSON.stringify(out, null, 2)); return; }
  ...
  out.before = {
    page: { qr_config: pageBefore.qr_config, published_revision: pageBefore.published_revision, published: pageBefore.published, public_id: pageBefore.public_id, title: pageBefore.title, page_type: pageBefore.page_type },
    profile: { public_id: profileBefore.public_id, published_revision: profileBefore.published_revision },
  };
  ...
  r = await fetch(base + '/rest/v1/pages?public_id=eq.' + PUBLIC_ID, {
    method: 'PATCH', headers: { ...uH, 'Prefer': 'return=representation' },
  ...
  r = await fetch(base + '/rest/v1/pages?select=qr_config&public_id=eq.' + PUBLIC_ID, { headers: uH });
  const ownerRead = await r.json();
  out.owner_select_qr = { status: r.status, qr_config: ownerRead[0] && ownerRead[0].qr_config };
  out.persist_owner_select = ownerRead[0] && JSON.stringify(ownerRead[0].qr_config) === JSON.stringify(SAVED_CONFIG);
  ...
  for (const f of PROFILE_QR_FIELDS) {
    ...
    if (!same) { out.profile_qr_isolation.changed = true; out.profile_qr_isolation.diff[f] = { before: profileBefore[f], after: profileAfter[f] }; }
  }
  ...
  r = await fetch(base + '/rest/v1/pages?public_id=eq.' + PUBLIC_ID, { method: 'PATCH', headers: anonH, body: JSON.stringify({ qr_config: { qr_foreground_color: '#ff0000' } }) });
  out.anon_update = { status: r.status, body: (await r.text()).slice(0, 120) };
  const pageAfterAnon = await getPage();
  out.anon_update_blocked = JSON.stringify(pageAfterAnon.qr_config) === JSON.stringify(SAVED_CONFIG);
  ...
  try {
    ...
    out.foreign_create = { status: r.status, id: foreignId, error: created.error || created.msg || null };
    ...
    if (foreignId) {
      ...
      out.foreign_signin = { status: r.status, uid: fauth.user && fauth.user.id, has_token: Boolean(fauth.access_token) };

      if (fauth.access_token) {
        const fH = { apikey: anon, Authorization: 'Bearer ' + fauth.access_token, 'Content-Type': 'application/json' };
        r = await fetch(base + '/rest/v1/pages?public_id=eq.' + PUBLIC_ID, { method: 'PATCH', headers: fH, body: JSON.stringify({ qr_config: { qr_foreground_color: '#00ff00' } }) });
    ...
    out.foreign_update_blocked = JSON.stringify(pageAfterForeign.qr_config) === JSON.stringify(SAVED_CONFIG);

    if (foreignId) {
      r = await fetch(base + '/auth/v1/admin/users/' + foreignId, { method: 'DELETE', headers: srH });
      out.foreign_cleanup = { status: r.status };
    }
  } catch (e) {
    out.foreign_error = String(e);
    if (foreignId) { await fetch(base + '/auth/v1/admin/users/' + foreignId, { method: 'DELETE', headers: srH }); }
  }

  // ---- 8. restore original qr_config (null) via owner ----
  r = await fetch(base + '/rest/v1/pages?public_id=eq.' + PUBLIC_ID, { method: 'PATCH', headers: { ...uH, 'Prefer': 'return=representation' }, body: JSON.stringify({ qr_config: null }) });
