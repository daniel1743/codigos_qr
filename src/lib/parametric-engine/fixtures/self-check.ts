...
export function runEngineSelfCheck(): SelfCheckResult {
  ...
  for (const { id, intent } of SAMPLE_INTENTS) {
    ...
    t(
      `fixture ${id}`,
      JSON.stringify(summarize(r)) === JSON.stringify(EXPECTED_RECIPES[id]),
  ...
  // 2. overrides
  const ov = generatePageRecipe(base, {
    now: NOW,
    overrides: {
      links_presentation: "buttons",
      identity_alignment: "center",
      density: "compact",
      visual_family: "minimal",
    },
  });
  ...
  t(
    "override density",
    ov.design.geometry.density === "compact" && ov.design.spacing.horizontal_padding !== "compact",
  );
  t("override family", ov.meta.family === "minimal");

  // 3. locked survives variants
  for (const v of [0, 1, 2, 3, 4, 5, 7, 11]) {
    const rc = generatePageRecipe(base, {
      now: NOW,
      variant: v,
      overrides: { links_presentation: "cards", locked: ["links_presentation"] },
    });
    t(
      `locked cards v${v}`,
      rc.structure.links.presentation === "cards",
      rc.structure.links.presentation,
    );
    const rb = generatePageRecipe(base, {
      now: NOW,
      variant: v,
      overrides: { links_presentation: "buttons", locked: ["links_presentation"] },
    });
    t(
      `locked buttons v${v}`,
      rb.structure.links.presentation === "buttons",
      rb.structure.links.presentation,
    );
    const rd = generatePageRecipe(base, {
      now: NOW,
      variant: v,
      overrides: {
        density: "spacious",
        identity_alignment: "left",
        locked: ["density", "identity_alignment"],
      },
    });
    t(
      `locked density/align v${v}`,
      rd.design.geometry.density === "spacious" && rd.structure.hero.identity_alignment === "left",
    );
  }
  ...
  const banner = {
    ...base,
    identity: { ...base.identity, banner_preview: "https://cdn.example.com/b.jpg" },
  };
  const hm = (mode: any, intent = banner) =>
    generatePageRecipe(intent, { now: NOW, overrides: { hero_mode: mode } }).structure.hero;
  t(
    "banner_avatar",
    hm("banner_avatar").mode === "banner_avatar" &&
      hm("banner_avatar").show_banner &&
      hm("banner_avatar").show_avatar,
  );
  ...
  t(
    "auto banner_avatar",
    generatePageRecipe(banner, { now: NOW }).structure.hero.mode === "banner_avatar",
  );
  t(
    "hero_banner capability fallback",
    generatePageRecipe(banner, {
      now: NOW,
      capabilities: { hero_banner: false },
      overrides: { hero_mode: "banner_only", locked: ["hero_mode"] },
  ...
  for (let v = 0; v < 8; v++) {
    ...
    sigs.add(
      [
        r.meta.family,
        r.structure.hero.mode,
        r.structure.links.presentation,
        r.structure.hero.identity_alignment,
        r.design.geometry.density,
        r.design.card.media_position,
      ].join("|"),
    );
  }
  ...
  const bad = (type: any, value: string) =>
    tryGeneratePageRecipe({ ...base, primary_action: { type, value } }).ok === false;
  ...
  t(
    ...
    tryGeneratePageRecipe({
      ...base,
      primary_action: { type: "instagram", value: "https://instagram.com/camila.ossa" },
    }).ok === true,
  );

  // 7. media capability fallback
  const noMedia = generatePageRecipe(base, {
    now: NOW,
    capabilities: { card_media_right: false, card_media_bottom: false },
  });
  t(
    "no unsupported media",
    noMedia.design.card.media_position === "none" &&
      !noMedia.blocks.some((b) => b.type === "media"),
  );
  const onlyRight = generatePageRecipe(base, {
    now: NOW,
    capabilities: { card_media_bottom: false },
  });
  t("bottom->right fallback", onlyRight.design.card.media_position === "right");

  // 8. CTA order on downgrade
  const noCards = generatePageRecipe(SAMPLE_INTENT_BY_ID["professional-leads-professional"]!, {
    now: NOW,
    capabilities: { professional_cards: false },
  });
  const types = noCards.blocks.map((b) => b.type);
  t(
    "cta before link_list",
    types.indexOf("primary_cta") < types.indexOf("link_list"),
    types.join(","),
  );
  ...
  t(
    "blob never serialized",
    rb2.identity.avatar === null && !JSON.stringify(rb2).includes("blob:"),
  );
  ...
  const opts = {
    variant: 3,
    overrides: { density: "compact" as const, locked: ["density" as const] },
  };
