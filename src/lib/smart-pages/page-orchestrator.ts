 ...
 * visual brain.
 *
 * SMART_PAGES_2 ADAPT notes:
 *  - Semantic page IDs (`page_*`, `site_*`) are TEMPORARY PLANNER identifiers.
 *    They are NOT database UUIDs, public_id values, QR identities or aliases.
 *  - `listings` is SEMANTIC ONLY: it can be planned but must never be persisted
 *    as a host `public.pages.page_type`.
 *  - No persistence, no Supabase, no public_id generation, no Engine V2 call.
 */
...
import type { NavItemV1 } from "./smart-pages.types";
...
/* ------------------------------------------- host page-type semantic mapping */

export interface PageTypeExperienceMapping {
  experienceType: ExperienceType;
  /** Human-readable note for unsupported / deferred mappings. */
  diagnostic?: string;
  /** true when the experience is semantic-only and must not become a DB page type. */
  semanticOnly?: boolean;
}

export function mapPageTypeToExperience(pageType: string): PageTypeExperienceMapping {
  switch (pageType.trim().toLowerCase()) {
    case "services":
      return { experienceType: "services" };
    case "catalog":
      return { experienceType: "catalog" };
    case "portfolio":
      return { experienceType: "portfolio" };
    case "menu":
      return { experienceType: "menu" };
    case "listings":
      return {
        experienceType: "listings",
        semanticOnly: true,
        diagnostic:
          "SEMANTIC_ONLY: 'listings' is not a host DB page type; defer it at the host boundary.",
      };
    case "promotion":
      return {
        experienceType: "landing",
        diagnostic:
          "MAPPED: 'promotion' has no dedicated Smart Pages experience; planned as landing.",
      };
    case "event":
      return {
        experienceType: "landing",
        diagnostic: "MAPPED: 'event' has no dedicated Smart Pages experience; planned as landing.",
      };
    case "campaign":
      return {
        experienceType: "landing",
        diagnostic:
          "MAPPED: 'campaign' has no dedicated Smart Pages experience; planned as landing.",
      };
    case "landing":
      return { experienceType: "landing" };
    default:
      return {
        experienceType: "landing",
        diagnostic: `UNSUPPORTED: '${pageType}' is not a recognized host page type; planned as landing.`,
      };
  }
}
