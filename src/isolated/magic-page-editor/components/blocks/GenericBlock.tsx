...
export function GenericBlock({ block, ctaVariants, maxWidth = 1080 }: GenericBlockProps) {
  ...
  const family = familyForBlockType(block.type);
  ...
  switch (block.type) {
    case "hero":
      return (
        <HeroFrame
          id={`block:${block.key}`}
          media={images.bioStill}
          mediaAlt="Imagen de portada"
        ...
        >
          {() => (
            <>
              ...
              <EditableText
                id={`${p}hero.sub`}
                value="Toca este texto para escribir tu mensaje principal."
                label="Subtítulo"
                className="cq-muted mt-3 max-w-md text-[15px]"
              />
            </>
          )}
        </HeroFrame>
    ...
    case "profile":
      return (
        <div className={cx(wrap, "flex flex-col items-center text-center")} style={{ maxWidth }}>
          <EditableAvatar
            id={`${p}avatar`}
            src={images.bioAvatar}
            alt="Foto de perfil"
            sizes={{ S: 72, M: 104, L: 136 }}
            ringColor={t.page.surface}
          />
          <EditableText
            id={`${p}profile.name`}
            value="Tu nombre"
            as="h2"
            label="Nombre"
            className={cx(heading, "mt-5")}
            style={display}
          />
          <EditableText
            id={`${p}profile.bio`}
            value="Una línea que explique a qué te dedicas."
            label="Descripción"
            className="cq-muted mt-2 text-[15px]"
          />
        </div>
      );
    ...
    case "text":
      return (
        <div className={wrap} style={{ maxWidth: Math.min(maxWidth, 760) }}>
          <EditableText
            id={`${p}text.title`}
            value="Un título para esta sección"
            as="h2"
            label="Título"
            className={heading}
            style={display}
          ...
          <EditableText
            id={`${p}text.body`}
            value="Escribe aquí un párrafo. Toca el texto para editarlo directamente, igual que cualquier otro elemento de la página."
            label="Párrafo"
            multiline
            className="cq-muted mt-4 text-[16px] leading-relaxed"
          />
        </div>
      );
    ...
    case "links":
      return (
        <div
          className={cx(wrap, "flex flex-col gap-3")}
          style={{ maxWidth: Math.min(maxWidth, 640) }}
        >
    ...
    case "gallery":
      return (
        <div className={wrap} style={{ maxWidth }}>
          <GalleryGrid
            id={`${p}gallery`}
            items={[images.bioStreet, images.bizClinic, images.pfStair]}
            defaultLayout="fila"
            radius={Math.min(t.radius, 20)}
            altPrefix="Foto"
          />
        </div>
      );
      ...
      return (
        <div className={wrap} style={{ maxWidth }}>
          ...
          >
            ...
            <span className="pointer-events-none absolute bottom-4 left-4 rounded-md bg-black/60 px-2 py-1 text-[12px] font-medium text-white">
              1:42
            </span>
          </EditableImage>
        </div>
    ...
    case "collection":
      return (
        <div className={wrap} style={{ maxWidth }}>
          <EditableText
            id={`${p}collection.title`}
            value="Colección"
            as="h2"
            label="Título"
            className={cx(heading, "mb-7")}
            style={display}
          />
          <div className={cx("grid gap-4", m ? "grid-cols-1" : "grid-cols-3")}>
            {genericCards.map((c, i) => (
              <Editable
                ...
                kind="card"
                label="Card"
                className="cq-surface flex flex-col overflow-hidden"
                style={{ borderRadius: t.radius }}
              >
                <EditableImage
                  id={`${p}collection.${i}.img`}
                  src={c.img}
                  alt={c.title}
                ...
                <div className="flex flex-1 flex-col p-5">
                  <EditableText
                    id={`${p}collection.${i}.title`}
                    value={c.title}
                    as="h3"
                    label="Título"
                    className="cq-fg text-[18px] font-semibold"
                  />
                  <EditableText
                    id={`${p}collection.${i}.desc`}
                    value={c.desc}
                    label="Descripción"
                    className="cq-muted mt-1.5 text-[14px]"
                  />
                </div>
              </Editable>
            ))}
