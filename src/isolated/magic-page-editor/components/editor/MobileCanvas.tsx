...
export function MobileCanvas() {
  ...
  useEffect(() => {
    ...
    const el =
      (ed.keyboard && ed.editingId ? ed.getElement(ed.editingId) : null) ??
      ed.getElement(ed.selection.id);
    if (!el) return;
    const timer = window.setTimeout(() => {
      ...
      if (outOfView)
        scroller.scrollBy({
          top: fits ? relTop - (visible - er.height) / 2 : relTop - 24,
          behavior: "smooth",
        });
    }, 80);
  ...
  const screen = (
    <div
      ...
      style={{ background: t.page.color }}
    >
      {!bare && (
        <div
          className="flex h-9 shrink-0 items-center justify-between px-6 text-[12.5px] font-semibold"
          style={{ color: t.page.fg }}
        >
          <span>9:41</span>
      ...
      <div
        ref={setScroller}
        className={cx(
          "cq-scroll-none relative min-h-0 flex-1 overflow-y-auto overflow-x-hidden",
        )}
      >
        <div
          ref={setContent}
    ...
    <div className="flex h-full w-full justify-center overflow-auto bg-canvas px-4 py-5">
      <div className="flex flex-col items-center gap-3">
        <div
          ...
          style={{
            width: ed.mobileWidth + 20,
            height: "min(860px, max(600px, calc(100vh - 190px)))",
          }}
        >
          <div className="h-full overflow-hidden rounded-[38px]">{screen}</div>
        </div>
        <div
          className="flex items-center gap-1 rounded-full bg-white p-1 shadow-[0_1px_2px_rgba(16,24,40,0.08)]"
          role="radiogroup"
          aria-label="Ancho del móvil"
        >
          {widths.map((w) => (
            <button
              key={w}
