...
export function FamilyCard({ id, family, item, blockProps, className, demo }: FamilyCardProps) {
  ...
  const pad =
    ({ S: 12, M: 20, L: 28 }[cp.spacing ?? (layout === "compact" ? "S" : "M")] ?? 20) - (m ? 4 : 0);
  const border =
    (cp.border ?? (surface === "plain" ? "off" : "on")) === "on" || surface === "outline";
  ...
  if (horizontal && !stackOnMobile) {
    ...
    content = (
      <div
        className={cx("flex flex-1 items-stretch", layout === "right" && "flex-row-reverse")}
        style={{ gap: m ? 14 : pad }}
      >
        {media("", {
          width: `${r}%`,
          minWidth: m ? 96 : 120,
          minHeight: m ? 104 : r === "25" ? 150 : 210,
        })}
    ...
    content = (
      <div className="flex flex-1 flex-col" style={{ gap: m ? 14 : 16 }}>
        {media(
          cx(
            "w-full",
            dense ? "aspect-square" : layout === "top" ? "aspect-[4/3]" : "aspect-[16/10]",
          ),
        )}
    ...
    content = (
      <div className="flex flex-1 flex-col" style={{ gap: 16 }}>
        <div
          data-slot="image"
          className={cx("grid grid-cols-2 gap-1.5", m ? "aspect-[4/3]" : "aspect-[21/9]")}
        >
  ...
  return (
    <Editable
      ...
      style={{
        ...
        boxShadow:
          [
            border ? "inset 0 0 0 1px var(--line)" : "",
            SHADOWS[cp.shadow ?? "none"] !== "none" ? SHADOWS[cp.shadow ?? "none"] : "",
          ]
            .filter(Boolean)
            .join(", ") || undefined,
        color: "var(--fg)",
