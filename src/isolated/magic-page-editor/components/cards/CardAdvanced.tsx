    ...
    <div className="space-y-5">
      <PanelSection title="Orden y visibilidad">
        <div className="flex gap-2">
          <button
            ...
            disabled={index <= 0}
            onClick={() =>
              ed.updateDoc((d) => moveCard(d, blockKey, family.items.length, itemId, -1))
            }
          >
          ...
          <button
            type="button"
            className={cx(btn, hidden && "border-select bg-select-soft text-select")}
            onClick={() => set("hidden", hidden ? "off" : "on")}
          >
      ...
      <PanelSection title="Campos">
        {sample.price !== undefined && (
          <Toggle
            label="Precio"
            checked={flag("showPrice")}
            onChange={(v) => set("showPrice", v ? "on" : "off")}
          />
        )}
        {sample.previousPrice !== undefined && (
          <Toggle
            label="Precio anterior"
            checked={flag("showPrev")}
            onChange={(v) => set("showPrev", v ? "on" : "off")}
          />
        )}
        {family.items.some((i) => i.badge) && (
          <Toggle
            label="Etiqueta"
            checked={flag("showBadge")}
            onChange={(v) => set("showBadge", v ? "on" : "off")}
          />
        )}
        {family.items.some((i) => i.cta) && (
          <Toggle
            label="Botón"
            checked={flag("showCta")}
            onChange={(v) => set("showCta", v ? "on" : "off")}
          />
        )}
      </PanelSection>
      ...
      <Toggle
        label="Borde"
        checked={(cp.border ?? (cp.surface === "plain" ? "off" : "on")) === "on"}
        onChange={(v) => set("border", v ? "on" : "off")}
      />
    </div>
  );
}
