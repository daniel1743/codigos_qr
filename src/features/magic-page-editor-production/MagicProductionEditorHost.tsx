...
export function MagicProductionEditorHost({ pageId }: { pageId: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const ownedPage = await magicPageService.getOwnedPage(supabase, pageId, data.session.user.id);
        if (!ownedPage) throw new Error("La página no existe o no pertenece al usuario autenticado.");
        ...
        if (active) setError(reason instanceof Error ? reason.message : "No se pudo cargar Magic Production.");
      } finally {
  ...
  const save = useCallback(async (state: MagicEditorStateV1) => {
  ...
  const onDocumentChange = useCallback((state: MagicEditorStateV1) => {
    setDocument(state);
    ...
    saveTimer.current = window.setTimeout(() => {
      void save(state).catch((reason) => setError(reason instanceof Error ? reason.message : "No se pudo guardar el borrador."));
    }, 650);
  }, [save]);

  const onPublish = useCallback(async (state: MagicEditorStateV1) => {
    if (!page || !session) return;
    await save(state);
  ...
  if (loading) return <div className="flex min-h-screen items-center justify-center">Cargando Magic Production…</div>;
    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-background px-6 text-foreground">
        <section className="max-w-lg rounded-2xl border border-border bg-card p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Magic Production</p>
          <h1 className="mt-2 text-xl font-semibold">Editor no disponible</h1>
          <p className="mt-2 text-sm text-muted-foreground">{error ?? "No se pudo validar la página solicitada."}</p>
        </section>
      </main>
    );
  }
