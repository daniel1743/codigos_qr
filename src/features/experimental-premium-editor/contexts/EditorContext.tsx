...
import { Category, HeroVariant, ImageOrigin, PageBackground, PageHeader, Product, Selection, TargetKind, TextStyle, ThemeTokens, CtaConfig, CardChrome, ProductOverrides } from '../types/editor';
...
interface EditorValue {
  ...
  patchSharedStyle: <K extends keyof EditorValue['sharedStyles']>(key: K, patch: Partial<EditorValue['sharedStyles'][K]>) => void;
...
export function EditorProvider({ children }: {children: React.ReactNode;}) {
  ...
  const [pageHeader, setPageHeader] = useState<PageHeader>({
    ...
    titleStyle: { font: 'Marcellus, Georgia, serif', size: 38, color: '#17140F', weight: 400, italic: false, underline: false, align: 'left' },
    description: 'Toca cualquier elemento de la tarjeta para editarlo en el sitio: imagen, título, descripción, precio o botón.',
    descriptionStyle: { font: 'Inter, system-ui, sans-serif', size: 14, color: '#4A443C', weight: 400, italic: false, underline: false, align: 'left' }
  });
  ...
  const patchPageHeader = useCallback((patch: Partial<PageHeader>) => setPageHeader(p => ({ ...p, ...patch })), []);
  const patchPageBackground = useCallback((patch: Partial<PageBackground>) => setPageBackground(p => ({ ...p, ...patch })), []);
  ...
  const patchTextStyle = useCallback(
    ...
    patch: Partial<TextStyle>) =>
    {
      setProducts((prev) => {
        ...
        const key = field === 'titleStyle' ? 'title' : field === 'descriptionStyle' ? 'description' : 'price';
        if (!p.overrides[key]) {
  ...
  const patchSharedStyle = useCallback(<K extends keyof EditorValue['sharedStyles']>(key: K, patch: Partial<EditorValue['sharedStyles'][K]>) => {
  ...
  const setOverride = useCallback((productId: string, key: keyof ProductOverrides, value: boolean) => {
    setProducts(prev => prev.map(p => {
  ...
  const bulkCloneProducts = useCallback((count: number) => {
    setProducts((prev) => {
    ...
    (id: string, file?: File) => {
      ...
      const timer = window.setTimeout(() => {
        ...
        p.id === id ?
        {
          ...
          imageState: 'ready',
          imageOrigin: 'own',
          image: localUrl ?? p.image ?? "https://images.unsplash.com/photo-1505693314120-0d443867891c?w=800&q=80"
        } :
        p
        )
        );
