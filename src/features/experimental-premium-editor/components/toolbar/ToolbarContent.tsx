...
const CTA_COLORS = ['#1E4D44', '#17140F', '#8A5A24', '#2F6FED', '#B42318', '#4A443C', '#7A736A', '#FFFFFF', '#F5F3F0', '#E6E1DA'];
...
export function ToolbarContent({ product, kind, level }: ToolbarContentProps) {
  ...
  <div className="flex shrink-0 items-center gap-0.5" role="group" aria-label="Alineación">
    ...
    ['right', AlignRightIcon, 'Derecha']] as
    const).
  ...
  <Popover
    ...
    <div>
          ...
          <div className="mt-3 border-t border-[#D6CFC5] pt-2">
            <button
              type="button"
              onClick={() => { editor.setInspectorOpen(true); close(); }}
  ...
  if (field) {
    ...
    []),
    <React.Fragment key="align">{alignGroup(style.align, (align) => apply({ align }))}</React.Fragment>];
  ...
  if (kind === 'badge') {
    ...
    const primary = [
      fontPopover(
        { font: category.style.font, size: category.style.size, color: category.style.textColor, weight: 400, italic: false, underline: false, align: 'left' },
        (patch) => {
      ...
      />,
      colorPopover(category.style.textColor, (color) => apply({ style: { ...category.style, textColor: color } }))
    ];
    ...
    return <Row level={level} primary={primary} secondary={secondary} more={moreMenu(sharedMore)} />;
  }
  ...
  if (kind === 'page' || kind === 'page-title' || kind === 'page-description') {
    if (kind === 'page-title' || kind === 'page-description') {
      ...
      const apply = (patch: Partial<TextStyle>) => editor.patchPageHeader({ [field]: { ...style, ...patch } });
    ...
    if (kind === 'page') {
        ...
        <Popover
          ...
          {(close) =>
            <div>
              ...
              <SwatchGrid
                colors={['#FFFFFF', '#FBF9F6', '#F5F2ED', '#EFEBE5', '#17140F', '#1E4D44', '#B42318', '#2F6FED']}
      ...
      return <Row level={level} primary={primary} secondary={[]} more={moreMenu(<MenuItem label="Ajustes avanzados" icon={<SlidersHorizontalIcon className="h-4 w-4" strokeWidth={1.7} />} onClick={() => editor.setInspectorOpen(true)} />)} />;
    }
  }
  ...
  if (kind === 'cta') {
    ...
    <React.Fragment key="align">
        {alignGroup(cta.align, (align) => editor.patchProduct(product.id, { cta: { ...cta, align } }))}
      </React.Fragment>];
