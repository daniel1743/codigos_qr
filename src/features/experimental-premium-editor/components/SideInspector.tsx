...
function AdvancedColorPicker({ color, onChange, label }: { color: string, onChange: (c: string) => void, label: string }) {
  return (
    <div className="flex flex-col gap-3">
      <p className="text-[13px] font-medium text-ink">{label}</p>
      <HexColorPicker color={color} onChange={onChange} style={{ width: '100%', height: '160px' }} />
      <div className="flex items-center gap-2">
        <div className="h-8 w-8 rounded-md border border-hairline shrink-0" style={{ backgroundColor: color }} />
        <input 
          type="text" 
          value={color} 
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex flex-col gap-1 max-h-[220px] overflow-y-auto pr-1">
        {filtered.map(f => (
          <button 
            ...
            className={cn(
              ...
              f.value === value ? "text-brand bg-brandSoft font-medium" : "text-ink hover:bg-[#F5F2ED]"
            )}
        ...
        {filtered.length === 0 && <p className="text-[12px] text-muted px-2 py-2">No se encontraron fuentes.</p>}
      </div>
    </div>
  );
}

function OverrideCheckbox({ label, checked, onChange }: { label: string, checked: boolean, onChange: (c: boolean) => void }) {
  return (
  ...
  return (
    <AnimatePresence>
      ...
      <motion.aside
        ...
        className="fixed right-0 top-16 z-30 h-[calc(100%-4rem)] w-[320px] border-l border-hairline bg-surface p-5 shadow-float lg:static lg:h-auto lg:shadow-none overflow-y-auto">
          ...
          <div className="mt-6 space-y-6">
            ...
            {selection?.kind === 'card' && selectedProduct && (
              <>
                <AdvancedColorPicker 
                  ...
                  onChange={(c) => editor.patchProduct(selectedProduct.id, { card: { ...selectedProduct.card, background: c } })} 
                />
                <OverrideCheckbox 
                ...
                <Field
                  ...
                  onChange={(longDescription) => editor.patchProduct(selectedProduct.id, { longDescription })} 
                />
              </>
            )}

            {(selection?.kind === 'title' || selection?.kind === 'description' || selection?.kind === 'price') && selectedProduct && (
              (() => {
                const field = selection.kind === 'title' ? 'titleStyle' : selection.kind === 'price' ? 'priceStyle' : 'descriptionStyle';
                const overrideKey = selection.kind === 'title' ? 'title' : selection.kind === 'price' ? 'price' : 'description';
                ...
                return (
                  <>
                    <AdvancedColorPicker 
                      ...
                      color={style.color} 
                      onChange={(c) => editor.patchTextStyle(selectedProduct.id, field, { color: c })} 
                    />
                    <FontLibrary 
                      value={style.font} 
                      onChange={(f) => editor.patchTextStyle(selectedProduct.id, field, { font: f })} 
                    />
                    <OverrideCheckbox 
                      ...
                      onChange={(checked) => editor.setOverride(selectedProduct.id, overrideKey, !checked)} 
                    />
                  </>
                );
              })()
            ...
            {selection?.kind === 'badge' && selectedProduct && (
              (() => {
                ...
                return (
                  <>
                    <AdvancedColorPicker 
                      ...
                      onChange={(c) => editor.patchCategory(category.id, { style: { ...category.style, backgroundColor: c } })} 
                    />
                    <AdvancedColorPicker 
                      label="Color del texto"
                      color={category.style.textColor} 
                      onChange={(c) => editor.patchCategory(category.id, { style: { ...category.style, textColor: c } })} 
                    />
                    <FontLibrary 
                      value={category.style.font} 
                      onChange={(f) => editor.patchCategory(category.id, { style: { ...category.style, font: f } })} 
                    />
            ...
            {selection?.kind === 'cta' && selectedProduct && (
              <>
                <AdvancedColorPicker 
                  ...
                  onChange={(c) => editor.patchProduct(selectedProduct.id, { cta: { ...selectedProduct.cta, color: c } })} 
                />
                <OverrideCheckbox 
