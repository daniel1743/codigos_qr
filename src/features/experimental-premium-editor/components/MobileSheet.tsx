...
export function MobileSheet() {
  ...
  return (
    <AnimatePresence>
      {selection && (selection.cardId === 'page' || selectedProduct) &&
      <motion.div
        key="sheet"
        data-chrome="true"
        role="dialog"
        ...
        dragElastic={0.14}
        onDragEnd={(_, info) => {
          if (info.offset.y < -28) setSheetSnap(NEXT[sheetSnap]);else
          if (info.offset.y > 28) setSheetSnap(PREV[sheetSnap]);
        }}
        className="fixed inset-x-0 bottom-0 z-40 flex flex-col rounded-t-3xl border-t border-hairline bg-white shadow-sheet lg:hidden">
          ...
          <div className="flex shrink-0 items-center justify-between gap-2 px-4 pb-2">
            ...
            <div className="flex shrink-0 items-center gap-1">
              ...
              onClick={() =>
              setSheetSnap(sheetSnap === 'expanded' ? 'medium' : NEXT[sheetSnap])
              }
