    ...
    >
      <div className="relative z-10 flex w-full max-w-[520px] flex-1 flex-col items-center pb-12 pt-0 sm:pb-16">
        ...
        <div className="w-full flex flex-col items-center px-4 sm:px-6">
          ...
          {/* Encabezado */}
          <div
            className={`text-center w-full px-4 mb-10 space-y-3 ${profile.avatar_shape === "none" && profile.banner_url ? "mt-8" : ""}`}
          >
            <h1
            ...
            {profile.bio && (
              <p
                className="mx-auto max-w-[320px] break-words text-base font-medium leading-relaxed opacity-80 md:text-lg"
                style={{ color: textColor }}
              >
                {profile.bio}
              </p>
          ...
          {/* Lista de Enlaces */}
          <div className="w-full space-y-3.5 flex-1 flex flex-col items-center">
            {links
              .filter((l) => l.enabled)
              .map((link, i) => {
                let btnClassName = `group relative flex w-full items-center justify-between p-4 px-5 text-left shadow-sm transition-all duration-200 hover:-translate-y-[1px] active:scale-[0.985] motion-reduce:transition-none motion-reduce:transform-none min-h-[56px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${radiusClass}`;
                ...
                if (buttonStyle === "solid" || buttonStyle === "pill") {
                  ...
                  btnClassName = `group relative flex w-full items-center justify-between p-4 px-2 text-left transition-all duration-200 hover:-translate-y-[1px] active:scale-[0.985] motion-reduce:transition-none motion-reduce:transform-none min-h-[56px] focus-visible:outline-none rounded-none border-b border-transparent bg-transparent hover:bg-black/5 shadow-none`;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative z-10 flex items-center gap-3.5 flex-1 min-w-0 pr-4">
                      {buttonStyle === "card" ? (
                      ...
                      <span
                        className={`break-words text-base font-semibold leading-snug ${buttonStyle === "minimal" || buttonStyle === "line" ? "text-lg tracking-tight" : ""}`}
                      >
                        {link.label || "Enlace"}
                      </span>
                    </div>

                    <div className="relative z-10 shrink-0 opacity-50 group-hover:opacity-100 transition-opacity">
