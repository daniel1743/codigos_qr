...
function getPreviewContent(templateId: string, previewData: GalleryPreviewData): BasicTemplateContent {
...
export function MobileTemplateGallery({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col justify-end lg:hidden">
      ...
      >
        {/* Handle */}
        <div className="w-full flex justify-center pt-3 pb-2" onClick={() => isPreviewing ? setPreviewTemplateId(null) : onClose()}>
        ...
        {!isPreviewing ? (
              ...
              {TEMPLATES.map((template) => {
                ...
                return (
                  <div key={template.id} className="flex flex-col gap-2">
                    <div className={`relative aspect-[9/16] w-full overflow-hidden rounded-xl border-2 transition-all ${isSelected ? 'border-primary' : 'border-border'}`}>
                      <TemplateThumbnail template={template} content={getDefaultContent(template.id)} />
                      {isSelected && (
                    ...
                    <div>
                      <p className="text-sm font-semibold truncate leading-tight">{template.name}</p>
                    </div>
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden relative">
            <div className="absolute top-4 right-4 z-50">
              <Button variant="secondary" size="sm" onClick={() => setPreviewTemplateId(null)} className="rounded-full shadow-md font-medium px-4">
