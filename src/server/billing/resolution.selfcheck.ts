...
function buildOwnershipDeps(state: OwnershipMockState): OwnershipResolutionDeps {
  return {
    ...
    async getBillingCustomerByProviderCustomerId(
    ...
    async getBillingCheckoutByProviderCheckoutId(
      provider: BillingProvider,
...
async function run(): Promise<void> {
  ...
  // trusted_context: trusted USER_A resolves USER_A.
  {
    ...
    assert(
      "trusted_context resolves USER_A",
      result.status === "RESOLVED" &&
        result.userId === "USER_A" &&
        result.source === "TRUSTED_CONTEXT",
    );
  }
  ...
  // existing_subscription: provider subscription resolves canonical owner.
  {
    ...
    const result = await resolveOwnership(deps, {
      provider: "stripe",
      providerSubscriptionId: "sub_1",
    });
    assert(
      "existing_subscription resolves USER_B",
      result.status === "RESOLVED" &&
        result.userId === "USER_B" &&
        result.source === "EXISTING_SUBSCRIPTION",
    );
  }

  // canonical_customer: known provider customer resolves canonical owner.
  {
    const state: OwnershipMockState = {
      subscriptions: new Map(),
      customers: new Map([["pc_9", mockCustomer("USER_C", "pc_9")]]),
      checkouts: new Map(),
    };
    const result = await resolveOwnership(buildOwnershipDeps(state), {
      provider: "stripe",
      providerCustomerId: "pc_9",
    });
    assert(
      "canonical_customer resolves USER_C",
      result.status === "RESOLVED" &&
        result.userId === "USER_C" &&
        result.source === "CANONICAL_CUSTOMER",
    );
  }
  ...
  // canonical_checkout: known provider checkout resolves canonical owner.
  {
    const state: OwnershipMockState = {
      subscriptions: new Map(),
      customers: new Map(),
      checkouts: new Map([["pchk_1", mockCheckout("USER_D", "pchk_1")]]),
    };
    const result = await resolveOwnership(buildOwnershipDeps(state), {
      provider: "stripe",
      providerCheckoutId: "pchk_1",
    });
    assert(
      "canonical_checkout resolves USER_D",
      result.status === "RESOLVED" &&
        result.userId === "USER_D" &&
        result.source === "CANONICAL_CHECKOUT",
    );
  }
  ...
  // no_owner: unknown references => OWNER_REQUIRED.
  {
    ...
    assert(
      "no_owner returns OWNER_REQUIRED",
      result.status === "OWNER_REQUIRED" && result.userId === null && result.source === null,
    );
  }
  ...
  // conflicting_owner: trusted USER_A + authoritative existing USER_B => OWNER_CONFLICT.
  {
    ...
    assert(
      "conflicting_owner returns OWNER_CONFLICT",
      result.status === "OWNER_CONFLICT" && result.userId === null,
    );
  }
  ...
  // provider_metadata: provider-metadata userId (raw provider id string) ignored.
  {
    ...
    const result = await resolveOwnership(deps, {
      provider: "stripe",
      providerCustomerId: "meta-USER_EVIL",
    });
    assert(
      "provider_metadata userId ignored",
      result.status === "OWNER_REQUIRED" && result.userId === null,
    );
  }
  ...
  // email: email never resolves ownership (provider customer ref that looks like email).
  {
    ...
    assert(
      "email never resolves ownership",
      result.status === "OWNER_REQUIRED" && result.userId === null,
    );
  }
  ...
  // configured provider reference resolves Pro / Business / Enterprise.
  {
    ...
    assert(
      "plan_mapping resolves Business",
      business.status === "RESOLVED" && business.planId === "business",
    );
    assert(
      "plan_mapping resolves Enterprise",
      enterprise.status === "RESOLVED" && enterprise.planId === "enterprise",
    );
  ...
  assert(
    "unknown_plan returns PLAN_MAPPING_REQUIRED",
    resolvePlanReference(registry, "stripe", "stripe_unknown").status === "PLAN_MAPPING_REQUIRED",
  );
  ...
  // conflicting_mapping: duplicate/conflicting mapping fails closed.
  {
    ...
    assert(
      "conflicting_mapping returns PLAN_MAPPING_CONFLICT",
      resolvePlanReference(conflicting, "stripe", "stripe_pro").status === "PLAN_MAPPING_CONFLICT",
    );
    assert(
      "conflicting_mapping resolver fails closed",
      createBillingPlanResolver(conflicting).resolvePlan("stripe", "stripe_pro") === null,
    );
  }
  ...
  // free: cannot map provider reference to free.
  {
    ...
    assert(
      "free cannot resolve",
      resolvePlanReference(freeRegistry, "stripe", "stripe_free").status !== "RESOLVED",
    );
    assert(
      "free resolver returns null",
      createBillingPlanResolver(freeRegistry).resolvePlan("stripe", "stripe_free") === null,
    );
  }
  ...
  // amount: amount never resolves plan (extra amount field ignored; no amount in result).
  {
    ...
    assert(
      "amount never resolves plan (still pro)",
      result.status === "RESOLVED" && result.planId === "pro",
    );
  ...
  {
    assert(
      "default_registry resolvePlanReference fails closed",
      resolvePlanReference(EMPTY_PLAN_MAPPING_REGISTRY, "stripe", "stripe_pro").status ===
        "PLAN_MAPPING_REQUIRED",
    );
    assert(
      "default_registry resolver resolves nothing",
      DEFAULT_PLAN_RESOLVER.resolvePlan("stripe", "stripe_pro") === null,
    );
  }
  ...
  // resolver satisfies Application Core plan-resolution seam.
  {
    ...
    assert(
      "resolver satisfies BillingPlanResolver seam",
      typeof resolver.resolvePlan === "function",
    );
    assert("resolver maps pro", resolver.resolvePlan("stripe", "stripe_pro") === "pro");
    assert(
      "resolver maps business",
      resolver.resolvePlan("stripe", "stripe_business") === "business",
    );
    assert(
      "resolver maps enterprise",
      resolver.resolvePlan("paypal", "paypal_ent") === "enterprise",
    );
...
console.log(
  `RESOLUTION SELFCHECK: ${passed} passed, ${failed} failed (${passed + failed} assertions).`,
);
if (failed > 0) {
  process.exit(1);
}
