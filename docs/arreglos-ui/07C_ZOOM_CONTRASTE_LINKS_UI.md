...
La propiedad Soft ahora cuenta con el mix alfa en JS puro: Se extrae el HEX base y se mezcla al 8% sobre el fondo, logrando evaluar contra el color _renderizado final_, impidiendo alertas falsas y garantizando warnings precisos.
...
- `src/lib/design/template-presets.ts` [MODIFIED] (Se corrigieron 3 presets base que generaban WARNING con el nuevo motor: _Beauty_, _Warm_ y _Sunset_ ajustando sutilmente su color de texto asignado por defecto para superar WCAG).
